...
public class ChainLightningModifier : BraveBehaviour
{
	...
	protected override void OnDestroy()
	{
		...
		this.ClearLink();
		if (this.BackLinkProjectile && this.BackLinkProjectile.GetComponent<ChainLightningModifier>())
		{
	...
	private void UpdateLinkToProjectile(Projectile targetProjectile)
	{
		...
		float num2 = BraveMathCollege.Atan2Degrees(vector.normalized);
		int num3 = Mathf.RoundToInt(vector.magnitude / 0.0625f);
		this.m_extantLink.dimensions = new Vector2((float)num3, this.m_extantLink.dimensions.y);
		this.m_extantLink.transform.rotation = Quaternion.Euler(0f, 0f, num2);
			...
			for (int i = 0; i < StaticReferenceManager.AllEnemies.Count; i++)
			{
				...
				if (!this.m_damagedEnemies.Contains(aiactor))
				{
					if (aiactor && aiactor.HasBeenEngaged && aiactor.IsNormalEnemy && aiactor.specRigidbody)
					{
						...
						bool flag2 = BraveUtility.LineIntersectsAABB(p1, p2, aiactor.specRigidbody.HitboxPixelCollider.UnitBottomLeft, aiactor.specRigidbody.HitboxPixelCollider.UnitDimensions, out zero);
						if (flag2)
						{
		...
		if (this.DamagesPlayers)
		{
			for (int j = 0; j < GameManager.Instance.AllPlayers.Length; j++)
			{
				...
				if (playerController && !playerController.IsGhost && playerController.healthHaver && playerController.healthHaver.IsAlive && playerController.healthHaver.IsVulnerable)
				{
					...
					bool flag3 = BraveUtility.LineIntersectsAABB(p1, p2, playerController.specRigidbody.HitboxPixelCollider.UnitBottomLeft, playerController.specRigidbody.HitboxPixelCollider.UnitDimensions, out zero2);
					if (flag3)
					{
