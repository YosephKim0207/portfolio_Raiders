...
public class ChargeBehavior : BasicAttackBehavior
{
	...
	public override BehaviorResult Update()
	{
		...
		if (this.leadAmount > 0f)
		{
			...
			vector2 = BraveMathCollege.GetPredictedPosition(vector, this.m_aiActor.TargetVelocity, this.m_aiActor.specRigidbody.UnitCenter, this.chargeSpeed);
	...
	public override ContinuousBehaviorResult ContinuousUpdate()
	{
		if (this.State == ChargeBehavior.FireState.Priming)
		{
			...
			if (this.m_timer > 0f)
			{
				...
				float num2 = BraveMathCollege.ClampAngle180(num - facingDirection);
				float num3 = facingDirection + Mathf.Lerp(0f, num2, this.m_deltaTime / (this.m_timer + this.m_deltaTime));
				this.m_aiAnimator.FacingDirection = num3;
			}
			if (!this.stopDuringPrime)
			{
				float magnitude = this.m_aiActor.BehaviorVelocity.magnitude;
				float num4 = Mathf.Lerp(magnitude, 0f, this.m_deltaTime / (this.m_timer + this.m_deltaTime));
				this.m_aiActor.BehaviorVelocity = BraveMathCollege.DegreesToVector(this.m_aiAnimator.FacingDirection, num4);
			}
