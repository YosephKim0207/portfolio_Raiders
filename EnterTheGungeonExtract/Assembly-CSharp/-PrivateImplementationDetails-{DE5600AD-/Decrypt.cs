...
namespace <PrivateImplementationDetails>{DE5600AD-6212-4D84-9A32-9D951E3289D1}
{
	...
	{
		// Token: 0x06009E37 RID: 40503 RVA: 0x003F2A88 File Offset: 0x003F0C88
		...
		// Token: 0x06009E38 RID: 40504 RVA: 0x003F2AEC File Offset: 0x003F0CEC
		public static string DecryptLiteral(byte[] b)
		{
			...
			if (b.Length == num2)
			{
				byte[] array = rsa.EncryptValue(b);
				string text = Decrypt.Translate(array);
			...
			for (int i = 0; i < b.Length / num2; i++)
			{
				...
				Buffer.BlockCopy(b, i * num2, array2, 0, num2);
				byte[] array3 = rsa.EncryptValue(array2);
				stringBuilder.Append(Decrypt.Translate(array3));
			}
		...
		private static byte[] publicKey = new byte[]
		{
			...
			181, byte.MaxValue, 17, 79, 20, 209, 81, 4, 124, 81,
			...
			132, 122, 204, 37, 72, 124, 117, byte.MaxValue, 25, 42,
			153, 181, 43, 71, 34, 48, 32, 162, 232, 243,
			21, 138, 4, 36, 235, 6, 60, 193, 198, 113,
			135, 142, 233, 34, 76, byte.MaxValue, 198, 118, 69, 27,
